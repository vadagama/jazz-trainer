...
describe('RBAC — resolvePermissions', () => {
  ...
  it('returns empty set for non-existent user', async () => {
    const perms = await resolvePermissions(db, 'no-such-user');
    expect(perms.size).toBe(0);
  });

  it('returns empty set for disabled user', async () => {
    const now = Date.now();
    db.insert(users)
      .values({
        id: 'disabled-user',
      ...
      .run();
    const perms = await resolvePermissions(db, 'disabled-user');
    expect(perms.size).toBe(0);
  });

  it('super_admin gets all seeded permissions', async () => {
    createUser(db, RBAC_ROLES.SUPER_ADMIN);
    const perms = await resolvePermissions(db, 'test-user');
  ...
  it('admin gets subset of permissions', async () => {
    createUser(db, RBAC_ROLES.ADMIN, 'admin-user');
    const perms = await resolvePermissions(db, 'admin-user');
    expect(perms.has(RBAC_PERMISSIONS.ADMIN)).toBe(true);
    expect(perms.has(RBAC_PERMISSIONS.USERS_READ)).toBe(true);
    expect(perms.has(RBAC_PERMISSIONS.CONTENT_READ)).toBe(true);
    expect(perms.has(RBAC_PERMISSIONS.CONTENT_WRITE)).toBe(true);
  ...
  it('regular user gets basic permissions', async () => {
    createUser(db, RBAC_ROLES.USER, 'plain-user');
    const perms = await resolvePermissions(db, 'plain-user');
  ...
  it('user-specific grant adds permission beyond role', async () => {
    ...
    db.insert(userPermissions)
      ...
      .run();
    const perms = await resolvePermissions(db, 'grant-user');
    expect(perms.has(RBAC_PERMISSIONS.CONTENT_READ)).toBe(true);
  });

  it('user-specific revoke removes role-based permission', async () => {
    ...
    db.insert(userPermissions)
      ...
      .run();
    const perms = await resolvePermissions(db, 'revoked-admin');
...
describe('RBAC — hasPermission', () => {
  ...
  it('returns true when user has permission via role', async () => {
    ...
    expect(await hasPermission(db, 'admin-1', RBAC_PERMISSIONS.ADMIN)).toBe(true);
  });

  it('returns false when user lacks permission', async () => {
    createUser(db, RBAC_ROLES.USER, 'user-1');
    expect(await hasPermission(db, 'user-1', RBAC_PERMISSIONS.ADMIN)).toBe(false);
  });

  it('returns false for non-existent user', async () => {
    expect(await hasPermission(db, 'no-one', RBAC_PERMISSIONS.ADMIN)).toBe(false);
  });
});
...
describe('RBAC — resolveFlags', () => {
  ...
  it('returns empty object when no flags exist', async () => {
    const flags = await resolveFlags(db, RBAC_ROLES.USER, 'user-1');
    expect(flags).toEqual({});
  });

  it('disabled flag returns false regardless of role', async () => {
    db.insert(featureFlags)
      ...
      .run();
    const flags = await resolveFlags(db, RBAC_ROLES.SUPER_ADMIN, 'user-1');
    expect(flags['feature-x']).toBe(false);
  });

  it('enabled flag without filters returns true for everyone', async () => {
    db.insert(featureFlags)
      .values({
        key: 'global-flag',
        enabled: true,
      ...
      .run();
    const flags = await resolveFlags(db, RBAC_ROLES.USER, 'any-user');
    expect(flags['global-flag']).toBe(true);
  });

  it('role-filtered flag returns true for matching role only', async () => {
    db.insert(featureFlags)
      .values({
        key: 'admin-only',
        enabled: true,
      ...
      .run();
    expect((await resolveFlags(db, RBAC_ROLES.ADMIN, 'u1'))['admin-only']).toBe(true);
    expect((await resolveFlags(db, RBAC_ROLES.USER, 'u1'))['admin-only']).toBe(false);
  });

  it('user-filtered flag returns true only for matching user', async () => {
    db.insert(featureFlags)
      .values({
        key: 'beta-tester',
        enabled: true,
      ...
      .run();
    expect((await resolveFlags(db, RBAC_ROLES.USER, 'user-a'))['beta-tester']).toBe(true);
    expect((await resolveFlags(db, RBAC_ROLES.USER, 'user-c'))['beta-tester']).toBe(false);
  });

  it('role + user filter: either condition grants access', async () => {
    db.insert(featureFlags)
      .values({
        key: 'hybrid',
        enabled: true,
      ...
      .run();
    // admin role matches
    expect((await resolveFlags(db, RBAC_ROLES.ADMIN, 'any'))['hybrid']).toBe(true);
    // user matches via userId
    expect((await resolveFlags(db, RBAC_ROLES.USER, 'special-user'))['hybrid']).toBe(true);
    // neither role nor userId matches
    expect((await resolveFlags(db, RBAC_ROLES.USER, 'other'))['hybrid']).toBe(false);
  });

  it('handles malformed JSON in roles gracefully', async () => {
    db.insert(featureFlags)
      .values({
        key: 'broken-json',
        enabled: true,
    ...
    const flags = await resolveFlags(db, RBAC_ROLES.ADMIN, 'u1');
    expect(flags['broken-json']).toBe(false);
  });

  // ── Percentage rollout (FEATURES-VISION.md §4.4) ──────────────────────────

  it('rolloutPercent=100 includes every user', async () => {
    db.insert(featureFlags)
      .values({ key: 'rollout-100', enabled: true, rolloutPercent: 100 })
      .run();
    expect((await resolveFlags(db, RBAC_ROLES.USER, 'user-a'))['rollout-100']).toBe(true);
    expect((await resolveFlags(db, RBAC_ROLES.USER, 'user-b'))['rollout-100']).toBe(true);
  });

  it('rolloutPercent=0 excludes every user', async () => {
    db.insert(featureFlags).values({ key: 'rollout-0', enabled: true, rolloutPercent: 0 }).run();
    expect((await resolveFlags(db, RBAC_ROLES.USER, 'user-a'))['rollout-0']).toBe(false);
  });

  it('rolloutPercent is deterministic for the same user', async () => {
    db.insert(featureFlags).values({ key: 'rollout-50', enabled: true, rolloutPercent: 50 }).run();
    const first = (await resolveFlags(db, RBAC_ROLES.USER, 'user-a'))['rollout-50'];
    const second = (await resolveFlags(db, RBAC_ROLES.USER, 'user-a'))['rollout-50'];
    expect(first).toBe(second);
  });

  it('rolloutPercent ignores role/user targeting (precedence)', async () => {
    ...
    db.insert(featureFlags)
      ...
      .run();
    expect((await resolveFlags(db, RBAC_ROLES.USER, 'any-user'))['rollout-precedence']).toBe(true);
  });

  it('rolloutPercent is ignored when flag is disabled', async () => {
    db.insert(featureFlags)
      .values({ key: 'rollout-disabled', enabled: false, rolloutPercent: 100 })
      .run();
    expect((await resolveFlags(db, RBAC_ROLES.USER, 'user-a'))['rollout-disabled']).toBe(false);
  });

  it('rolloutPercent approximates even distribution over many users', async () => {
    db.insert(featureFlags)
    ...
    for (let i = 0; i < total; i++) {
      if ((await resolveFlags(db, RBAC_ROLES.USER, `user-${i}`))['rollout-dist']) enabled++;
    }
  ...
  it('expired flag returns false even when enabled', async () => {
    db.insert(featureFlags)
      .values({ key: 'expired', enabled: true, expiresAt: Date.now() - 1000 })
      .run();
    expect((await resolveFlags(db, RBAC_ROLES.SUPER_ADMIN, 'u1'))['expired']).toBe(false);
  });

  it('non-expired flag with future expiresAt resolves normally', async () => {
    db.insert(featureFlags)
      .values({ key: 'future-expiry', enabled: true, expiresAt: Date.now() + 60_000 })
      .run();
    expect((await resolveFlags(db, RBAC_ROLES.USER, 'u1'))['future-expiry']).toBe(true);
  });
});
