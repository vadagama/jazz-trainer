  ...
  fastify.patch<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      if (role.name === RBAC_ROLES.SUPER_ADMIN) {
        return reply.status(403).send({ error: { code: 'FORBIDDEN', message: 'Cannot modify super_admin role' } });
      }
      ...
      const updated = withAuditSync(db, request, 'role.update', 'role', id, {}, () => {
        ...
        return {
          ...
          permissions: rps.map((rp) => rp.code),
          inactivePermissions: [],
          createdAt: refreshed.createdAt instanceof Date ? refreshed.createdAt.getTime() : (refreshed.createdAt as number),
        };
      });
  ...
  fastify.delete<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      if (
        ...
        return reply.status(403).send({ error: { code: 'FORBIDDEN', message: 'Cannot delete a system role' } });
      }
