...
> **Цель:** Web + Desktop + MIDI на одном ядре.
>
> **Оценка:** 5–10 дней. 3 PR.
...
### 8.3. Desktop-оболочка

- [ ] **5.3** `apps/desktop/` — Electron (или Tauri):
  - Переиспользует `apps/web` для UI
  - `native-adapter` для системного MIDI (если Web MIDI недостаточно)
  - Упаковка под macOS/Windows/Linux
...
- [x] Desktop-приложение запускается и идентично web-версии по функциональности
