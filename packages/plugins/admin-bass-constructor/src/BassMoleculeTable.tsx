...
import type { BassArticulation, BassHit } from '@jazz/music-core';
import {
  clamp01,
  colLabel,
  colToTick,
...
type Props = MoleculeEditorProps<'swing' | 'bossa' | 'funk' | 'latin' | 'ballad', BassArticulation>;

/**
 * Бас-конструктор использует 32-е ноты (subdivisions=8) для более мелкой сетки
 * (как барабаны), чтобы плотные funk/latin молекулы можно было редактировать.
 * Swing/ballad с 32-ми дают 8×4 = 32 колонки/такт; funk/latin — 8×4 = 32.
 */
const BASS_SUBDIVISIONS = 8;
...
export function BassMoleculeTable({
  molecule,
  onChange,
  onDelete,
  isPlaying,
  ...
  const perBar = colsPerBar(style, BASS_SUBDIVISIONS);
  const totalCols = perBar * bars;
  const tickStep = ticksPerCol(style, BASS_SUBDIVISIONS);
  ...
  /**
   * Rows = 4 артикуляции (виды звукоизвлечения): regular, muted, rel, stac.
   * Ступени (root/fifth/third…) движок выбирает сам — в конструкторе их нет.
   * Стабилен независимо от того, какие артикуляции уже есть в молекуле.
   */
  const rows = ALL_BASS_ARTICULATIONS.map((artic) => ({
    sound: artic,
    label: articulationLabel(artic),
  }));

  /** Артикуляции, у которых уже есть хотя бы один атом (для подсветки строк). */
  ...
  molecule.atoms.forEach((a, i) => {
    const col = tickToCol(a.atTick, style, BASS_SUBDIVISIONS);
  ...
  for (const a of molecule.atoms) {
    const startCol = tickToCol(a.atTick, style, BASS_SUBDIVISIONS);
  ...
  const addAtom = (sound: string, col: number) => {
    onChange({
      ...moleculeRef.current,
      atoms: [
        ...moleculeRef.current.atoms,
        makeAtom(sound as BassArticulation, colToTick(col, style, BASS_SUBDIVISIONS)),
      ],
    });
  };
  ...
  const moveAtom = (sound: string, fromCol: number, toCol: number) => {
    ...
    onChange({
      ...
      atoms: moleculeRef.current.atoms.map((a, i) =>
        i === idx ? { ...a, atTick: colToTick(toCol, style, BASS_SUBDIVISIONS) } : a,
      ),
    });
  ...
  const onCellEnter = (sound: string, col: number) => {
    // Драг остаётся в той артикуляции (строке), где начался.
  ...
  const playheadCol =
    currentTick != null && currentTick >= 0
      ? tickToCol(currentTick, style, BASS_SUBDIVISIONS) % totalCols
      : -1;
          ...
          <Button
            ...
            onClick={
              isPlaying
                ...
                : () =>
                    onPlay(
                      molecule.atoms.map((a) => ({ ...a }) as BassHit),
                      molecule.bars,
                    )
            }
          >
          ...
          <div className="w-full" style={{ minWidth: LABEL_W + totalCols * 32 }}>
            ...
            >
              ...
              {Array.from({ length: totalCols }, (_, col) => (
                ...
                >
                  {colLabel(col, style, BASS_SUBDIVISIONS)}
                </div>
              ))}
            </div>

            {/* ── Articulation rows (4 fixed) ─────────────────────────── */}
            {rows.map((row) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    ...
                    title={articulationDescription(row.sound)}
                  >
                    {row.label}
                  </div>
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          // без него ноты на смежных колонках визуально сливаются в блок.
