/**
 * Rhodes molecule editor — piano-roll grid over voicing roles.
 *
 * Mirrors PianoMoleculeTable, but uses {@link RhodesVoicingRole} (VoiceRole +
 * arp1..arp4) as the row set. Click=add/remove, drag↔=move, Shift+↔=duration,
 * drag↕=velocity. See docs/RHODES.md.
 */
...
export function RhodesMoleculeTable({
  molecule,
  onChange,
  onDelete,
  isPlaying,
  ...
  /** Rows: fixed canonical role order (RHODES_ROLES) — stable regardless of
   *  which roles currently have atoms. arp4..arp1 on top, then upper/top/
   *  shell/chord/bass — mirroring a piano-roll (high → low). */
  const rows = RHODES_ROLES.map((role) => ({ sound: role, label: roleLabel(role) }));
  ...
  const addAtom = (sound: string, col: number) => {
    onChange({
      ...
      atoms: [
        ...
        makeAtom(sound as RhodesVoicingRole, colToTick(col, style)),
      ],
    });
  };
          ...
          <Button
            ...
            onClick={
              isPlaying
                ...
                : () =>
                    onPlay(
                      molecule.atoms.map((a) => ({ ...a }) as RhodesHit),
                      molecule.bars,
                    )
            }
          >
