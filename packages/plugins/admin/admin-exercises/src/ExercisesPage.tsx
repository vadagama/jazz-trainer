...
import { EXERCISE_FEATURES } from '@jazz/shared';
...
export default function ExercisesPage() {
  ...
  const filteredFeatures = useMemo(() => {
    ...
    const list = EXERCISE_FEATURES.filter((f) => !UMBRELLA_CODES.has(f.code));
    return q
      ? list.filter(
  ...
  const allFeatures = useMemo(
    () => EXERCISE_FEATURES.filter((f) => !UMBRELLA_CODES.has(f.code)),
    [],
  );
