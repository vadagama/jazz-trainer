...
export interface AdminFilterState {
  ...
  tags: string[];
  author: string;
  /** Entry visibility status filter */
  visibility: string[];
...
export function AdminFilters({ value, onChange }: Props) {
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <MultiSelect
        ...
        onToggle={(v) => onChange({ ...value, visibility: toggle(value.visibility, v) })}
        renderLabel={(v) => VISIBILITY_LABEL[v] ?? v}
      />
      <MultiSelect
        label="Стиль"
        options={STYLES}
