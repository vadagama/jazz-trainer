...
import type { DrumHit, DrumMolecule, DrumPatternStyle, DrumSound } from '@jazz/music-core';
import {
  colLabel,
  colToTick,
  colsPerBar,
...
interface MoleculeTableProps {
  molecule: DrumMolecule;
  onChange: (next: DrumMolecule) => void;
  onDelete: () => void;
  isPlaying: boolean;
...
function atomsAt(molecule: DrumMolecule, sound: DrumSound, col: number, style: DrumPatternStyle): number[] {
  const idx: number[] = [];
...
export function MoleculeTable({ molecule, onChange, onDelete, isPlaying, onPlay, onStop }: MoleculeTableProps) {
  ...
  const selectedAtomIdx = selected != null ? atomsAt(molecule, selected.sound, selected.col, style) : [];
  ...
  const addAtom = (sound: DrumSound, col: number) =>
    onChange({ ...moleculeRef.current, atoms: [...moleculeRef.current.atoms, makeAtom(sound, colToTick(col, style))] });

  const removeAtom = (sound: DrumSound, col: number) => {
    const existing = atomsAt(moleculeRef.current, sound, col, style);
    if (existing.length === 0) return;
    onChange({ ...moleculeRef.current, atoms: moleculeRef.current.atoms.filter((_, i) => !existing.includes(i)) });
  };
  ...
  const setVelocityAt = (sound: DrumSound, col: number, v: number) => {
    ...
    onChange({
      ...moleculeRef.current,
      atoms: moleculeRef.current.atoms.map((a, i) => (idx.includes(i) ? { ...a, velocity: clamped } : a)),
    });
  };
  ...
  const onCellPointerDown = (sound: DrumSound, col: number, active: boolean, e: React.PointerEvent) => {
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between gap-2 space-y-0">
        ...
        <div className="flex items-center gap-1">
          <Button size="sm" variant={isPlaying ? 'secondary' : 'default'} onClick={isPlaying ? onStop : play}>
                    ...
                    return (
                      ...
                      >
                        <button
                          ...
                          title={active ? `velocity ${vel.toFixed(2)} · тяни вверх/вниз · 2×клик — удалить` : 'клик — добавить удар'}
                        />
                      </td>
                    );
                  })}
        ...
        {selected && (
          <div className="flex flex-wrap items-center gap-4 rounded-md border border-border bg-muted/30 p-3 text-sm">
            <span className="font-medium">
              {soundLabel(selected.sound)} · {colLabel(selected.col % perBar, style)}
