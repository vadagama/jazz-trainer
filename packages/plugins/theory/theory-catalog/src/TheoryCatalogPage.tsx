...
export function TheoryCatalogPage() {
  ...
  const stateByCode = useMemo(() => {
    ...
    const map = new Map<string, 'active' | 'inactive' | 'hidden'>();
    for (const l of LECTURES) {
      if (activeSet.has(l.featureCode)) map.set(l.featureCode, 'active');
      else if (inactiveSet.has(l.featureCode)) map.set(l.featureCode, 'inactive');
      else map.set(l.featureCode, 'hidden');
    }
    return map;
  ...
  const visibleLectures = useMemo(() => {
    return LECTURES.filter((l) => stateByCode.get(l.featureCode) !== 'hidden');
  }, [stateByCode]);
  ...
  const sorted = useMemo(() => {
    ...
    items.sort((a, b) => {
      ...
      const sb = stateByCode.get(b.featureCode) === 'active' ? 0 : 1;
      return sa - sb;
    });
    return items;
  }, [filtered, sort, counts, stateByCode]);
