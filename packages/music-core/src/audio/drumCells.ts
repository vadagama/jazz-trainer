...
const CELLS_SOURCE: Record<string, DrumCell> = GENERATED_DRUM_CELLS as Record<string, DrumCell>;
...
for (const [id, cell] of Object.entries(CELLS_SOURCE)) {
  ...
  if (errs.length > 0) {
    CELL_VALIDATION_ERRORS.push(`${id}: ${errs.map((e) => `${e.code}(${e.detail})`).join(', ')}`);
  }
}
