...
export function useEffectiveSettings(): UserSettingsDTO {
  ...
  if (defaultSettings) {
    ...
    const perStyleKeys = new Set([
      'bassEnabled', 'bassVolume', 'bassVariant', 'bassTension', 'bassHumanize',
      'bassPattern', 'bassRange', 'bassComplexity', 'bassUseMutedNotes',
      'pianoEnabled', 'pianoVolume', 'pianoPattern', 'pianoHumanize',
      'pianoVoicingDensity', 'pianoSampleLibrary', 'pianoTension', 'pianoRandomizationLevel',
      'rhodesEnabled', 'rhodesVolume', 'rhodesPattern',
      'rhodesVoicingDensity', 'rhodesMode', 'rhodesLayerMode', 'rhodesLayerVolume',
      'drumsEnabled', 'drumsVolume', 'drumKit', 'drumsPattern',
      'drumsHumanizeIntensity', 'drumsBassDrumEnabled', 'drumsBassDrumVolume',
      'drumsSnareEnabled', 'drumsSnareVolume',
      'drumsHihatEnabled', 'drumsHihatVolume', 'drumsHihatOpenness',
      'drumsRideEnabled', 'drumsRideVolume',
      'drumsCrashEnabled', 'drumsCrashVolume', 'drumsCrashFrequency',
      'drumsRimEnabled', 'drumsRimVolume',
      'drumsTomEnabled', 'drumsTomVolume',
      'percussionEnabled', 'percussionVolume', 'percussionPattern',
      'percussionHumanizeIntensity',
      'percussionCongaHighEnabled', 'percussionCongaHighVolume',
      'percussionCongaLowEnabled', 'percussionCongaLowVolume',
      'percussionBongoLowEnabled', 'percussionBongoLowVolume',
      'percussionTumbaEnabled', 'percussionTumbaVolume',
      'percussionTimbalesEnabled', 'percussionTimbalesVolume',
      'percussionCowbellEnabled', 'percussionCowbellVolume',
      'percussionClaveEnabled', 'percussionClaveVolume',
      'percussionShakerEnabled', 'percussionShakerVolume',
      'percussionGuiroEnabled', 'percussionGuiroVolume',
      'percussionCabasaEnabled', 'percussionCabasaVolume',
      'percussionTriangleEnabled', 'percussionTriangleVolume',
      'percussionTambourineEnabled', 'percussionTambourineVolume',
      'percussionVibraslapEnabled', 'percussionVibraslapVolume',
      'percussionBelltreeEnabled', 'percussionBelltreeVolume',
      'percussionWhistleEnabled', 'percussionWhistleVolume',
      'percussionSleighBellsEnabled', 'percussionSleighBellsVolume',
      'guitarEnabled', 'guitarVolume',
      'bpm', 'swingRatio',
    ]);

    const perStyleOverrides = {
      ...((merged as Record<string, unknown>).perStyleOverrides as Record<string, Record<string, unknown>> ?? {}),
    };
