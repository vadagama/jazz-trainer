...
function assignChordIds(content: GridContent): GridContent {
  ...
  const bars = content.bars.map((bar) => ({ ...bar, chords: withIds(bar.chords) }));
  const sections = content.sections?.map((s) => ({
    ...s,
    bars: s.bars.map((bar) => ({ ...bar, chords: withIds(bar.chords) })),
  }));
  return { ...content, bars, ...(sections ? { sections } : {}) };
}
