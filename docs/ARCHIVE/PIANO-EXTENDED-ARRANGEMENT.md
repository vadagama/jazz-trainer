...
> **Дата:** 2026-07-24
> **Из v1 перенесено:** опорная таблица надстроек (§5.2), таблица типов проходящих аккордов (§5.4) — адаптированы под 3-слойную архитектуру v2.
...
1. **Ритм** — молекула описывает _когда_, _как громко_, _какой длительности_ и _какую роль голоса_ играть. Больше **никаких абсолютных интервалов** в атомах.
2. **Гармония** — voicing-движок (`buildPianoVoicing` + `suggestUpperStructure`) резолвит _какие ноты_ звучат, исходя из **реального качества аккорда** под тактом и **одной пользовательской настройки** — уровня напряжения (`tension`).
...
**Опорная таблица надстроек (US_TABLE):**
...
**Опорная таблица типов подходов (теги `fill`):**

| Тег               | Подход                           | Пример для Cmaj7 |
| ----------------- | -------------------------------- | ---------------- |
| `chromatic-above` | Хроматический сверху (♭IIm7 → I) | D♭m7 → Cmaj7     |
| `chromatic-below` | Хроматический снизу (VII7 → I)   | B7 → Cmaj7       |
| `diatonic`        | Диатонический (ii7 → V7 → I)     | Dm7 → G7 → Cmaj7 |
| `diminished`      | Diminished (#Idim7 → ii7)        | C#dim7 → Dm7     |
| `dominant-chain`  | Dominant chain (V7/ii → ii7)     | A7 → Dm7         |
...
2. Обновить `docs/Instruments/PIANO.md` (разделы §4, §6, §8, §9).
   > Оценка: 2–3 ч.
...
_Документ обновлён: 2026-07-24. Замещает `docs/PIANO-EXTENDED-ARRANGEMENT.md` (v1)._
_Связанные документы: `docs/Instruments/PIANO.md` (текущая реализация), `docs/Instruments/RHODES.md` (комплементарный слой), `docs/Instruments/DRUMS.md` (общий pattern-движок)._
