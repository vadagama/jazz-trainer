...
**Ключевая ценность:** Пользователь выбирает лад (например, «D Дорийский») → нажимает Play → слышит полноценный аккомпанемент с басом, фортепиано и барабанами, построенный на аккордах этого лада. Это превращает Amazilia из «справочника ладов» в **тренажёр импровизации под ладовый аккомпанемент** — уникальную нишу, не покрытую конкурентами.
...
| **Amazilia (цель)** | **51 лад, 7 категорий** | **Автоматическая гармонизация: pedal/vamp/progression** | **Да (через midiEval)**  | **Да: все 7 мел. минора, altered, diminished** |

**Уникальная ниша:** Amazilia — **единственный** инструмент, который автоматически генерирует аккомпанемент под выбранный лад. Конкуренты либо показывают лад визуально, либо играют готовые сетки, но никто не делает мост «лад → аккорды → живой аккомпанемент» в реальном времени.
...
_Документ создан 2026-06-16. Отражает видение подсистемы ладов и их аккордового аккомпанемента для Amazilia. После принятия — станет основой для технической реализации в `music-core/src/scales/`._
