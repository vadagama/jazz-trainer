# Роли и права доступа — Amazilia

Подробный справочник по ролям, разрешениям (permissions) и модели доступа в Amazilia.
...
Всего **29 разрешений**, сгруппированных по доменам. Каждое разрешение — строка вида `domain:action`.
...
### 2.13. Биллинг и подписки

| Код              | Название                | Описание                                                   |
| ---------------- | ----------------------- | ---------------------------------------------------------- |
| `billing:read`   | Чтение подписок         | Просмотр списка подписок и заявок в админ-панели           |
| `billing:manage` | Управление подписками   | Активация, отмена, изменение подписок; обработка заявок    |

---

## 3. Роли

В системе **7 ролей**. Каждая роль — набор разрешений, определённый в `SEED_ROLES` (`apps/api/src/db/seed.ts`).

### 3.1. `super_admin` — Суперадминистратор

**Имеет все 29 разрешений.** Полный доступ ко всей системе, включая управление ролями, пользователями, системными настройками и аудитом.

**Разрешения:** `ALL_PERMISSIONS` (все 29).
...
**Имеет 26 разрешений.** Административный доступ к большинству разделов, но без права на критические операции.
...
- `billing:read`, `billing:manage` — управление подписками
...
### 3.3. `subscriber_free` — Бесплатный подписчик

**Имеет 4 разрешения.** Базовый уровень для всех зарегистрированных пользователей с бесплатной подпиской.

**Разрешения:**

- `catalog:read` — просмотр каталога
- `compositions:read` — просмотр композиций
- `profile:read`, `profile:write` — управление профилем
...
### 3.4. `subscriber_pro` — Pro-подписчик

**Имеет разрешения `subscriber_free` + полный доступ к упражнениям и созданию контента.**

**Разрешения:**

- Все разрешения `subscriber_free`
- `compositions:write` — создание композиций
- Все коды фич (`ALL_FEATURE_CODES`): упражнения, теория, ear training

**Не имеет:**

- ❌ `admin` — нет доступа к админке
- ❌ Все административные разрешения

### 3.5. `subscriber_premium` — Premium-подписчик

**Имеет все разрешения `subscriber_pro`.** Зарезервировано для будущих premium-фич (MIDI-оценка, rhythm drills, персональный план).

**Разрешения:** как `subscriber_pro`.
...
### 3.6. `catalog_editor` — Редактор каталога
...
- ❌ `billing:read`, `billing:manage` — нет доступа к управлению подписками
- ❌ `system:settings:read`, `system:settings:write` — нет доступа к системным настройкам

### 3.7. `user` — Обычный пользователь
...
| Разрешение              | `super_admin` | `admin` | `catalog_editor` | `user` | `sub_free` | `sub_pro` | `sub_premium` |
| ----------------------- | :-----------: | :-----: | :--------------: | :----: | :--------: | :-------: | :-----------: |
| **Администрирование**   |               |         |                  |        |            |           |               |
| `admin`                 |      ✅       |   ✅    |        ✅        |   ❌   |     ❌     |    ❌     |      ❌       |
| **Пользователи**        |               |         |                  |        |            |           |               |
...
_Документ основан на коде: `rbac.service.ts` v.2026-07, `seed.ts` v.2026-07. Обновлено 2026-07-26._
