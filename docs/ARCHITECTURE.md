...
> Статус: **целевая архитектура, частично реализована**.
>
> Выполнено: Фазы 0 (границы), 1 (SDK + Host), 2 (AudioPort — контракты), 3 (фичи → плагины), R (RBAC + аудит), 4 (новые домены — scaffold).
> В процессе: Фаза 2 (адаптеры — wiring), Фаза 4 (наполнение плагинов контентом), ActivityRunner.
> Не начато: Фаза 5 (MIDI + Desktop), командная палитра, настройки через settingsSchema.
>
> Документ описывает целевую архитектуру и текущее состояние — что уже реализовано, что в процессе, и как дойти до цели. Ниже по тексту: 🟢 = реализовано, 🟡 = частично, 🔴 = запланировано.
>
...
graph TD
    subgraph edge["Платформенные адаптеры (край)"]
        ...
        A2["🔴 webmidi-adapter (MIDI out/in) — stub"]
...
| плагины (`plugins/*`) | `packages/plugins/*` | `sdk`, `core` | другие плагины, shell, host напрямую |
| адаптеры | `packages/adapters/*` | `sdk`, `core` | плагины |
| shell | `apps/web` | host, sdk, core, shared | внутренности плагинов |
...
graph LR
    ...
    subgraph technique["Тренировка техник (technique)"]
...
| `ear-training` | technique | `/ear-training` | practice → Ear Training |
| `rhythm-drills` | technique | `/rhythm-drills` | practice → Rhythm Drills |
...
**🟡 Частично:**
- `tone-audio-adapter` и `webmidi-adapter` — пакеты созданы, экспортируют заглушки
- `useTransport` всё ещё живёт в `apps/web/src/engine/useTransport.ts`, не обёрнут в адаптер

**🔴 Запланировано:**
- Wiring: `tone-audio-adapter` реализует `AudioPort`, оборачивая текущий `useTransport`
- `webmidi-adapter` реализует `AudioPort` + `InputPort`
- `native-adapter` для desktop
...
│   │
│   ├── adapters/
│   │   ├── tone-audio-adapter/       🟡 stub: экспортирует ToneAudioAdapter
│   │   └── webmidi-adapter/          🔴 stub: экспортирует WebMidiAdapter
│   │
│   ├── plugins/
...
│
├── docs/                             ARCHITECTURE.md, PLAN.md, PHASE-*-REPORT.md, ...
...
Фактический охват тестами (на 2026-06-11, 348 тестов):
...
| Контрактные | `plugin-sdk`: validateManifest (8 тестов). `music-core`: testAudioPortContract | 🟢 8 + контракт |
...
```jsonc
// eslint-plugin-boundaries: матрица зависимостей (8 правил, default: disallow)
{
  ...
  "from": "packages/plugins/*",    "allow": ["plugin-sdk", "music-core", "shared"] },
  "from": "packages/adapters/*",   "allow": ["plugin-sdk", "music-core", "shared"] },
  "from": "apps/web",              "allow": ["apps/web", "plugin-host", "plugin-sdk", "music-core", "shared"] },
  "from": "apps/api",              "allow": ["apps/api", "music-core", "shared"] }
```

Плюс `import/no-restricted-paths`: запрет `apps/* → music-core`, `apps/* → shared`, `apps/web/src/shell → plugins`.
...
| Ф2 — AudioPort | 🟡 | в процессе | Порты определены (`ports.ts`), контрактный тест есть, адаптеры — stub. Нужен wiring: обернуть `useTransport` в `tone-audio-adapter` за `AudioPort` |
| Ф3 — Фичи → плагины | ✅ | 2026-06-11 | `core-editor`, `core-player`, `catalog` вынесены. App.tsx рендерит из `contributions` |
| Ф4 — Новые домены | 🟡 | scaffold готов | 10 domain-плагинов созданы и зарегистрированы. Наполнение контентом — в процессе |
| Ф5 — MIDI + Desktop | 🔴 | не начато | `webmidi-adapter` — stub, `apps/desktop` не создан |
...
| Звук/MIDI | Порты + адаптеры | 🟡 Порты готовы, адаптеры — stub |
...
| Тесты | Контракты + чистое ядро + тонкий E2E | 🟡 Unit + контрактные есть, E2E нет |
...
*Документ обновлён 2026-06-12. Отражает фактическое состояние кодовой базы на коммитах Фаз 0, 1, R, 3 и частично 2/4.*
