...
Amazilia собирает данные, но не использует их. Таблица `audit_log` спроектирована (ADR-007), сервис `withAudit()` готов — но **ни разу не вызван** (D-011 в `TECH_DEPT.md`). Страница диагностики (`admin-diagnostics`) — заглушка. Ноль видимости в поведение пользователей.
...
**Вывод:** Ниша свободна. Ни один конкурент в музыкальном EdTech не даёт админу/преподавателю аналитику по активности учеников. Amazilia может быть первым — особенно в сценарии «преподаватель следит за группой студентов».
